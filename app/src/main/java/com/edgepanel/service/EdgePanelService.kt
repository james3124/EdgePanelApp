...
class EdgePanelService : Service() {
    ...
    private val PANEL_WIDTH_NORMAL = 320
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            ACTION_RELOAD_HANDLE -> { handleConfig = Prefs.loadHandleConfig(this); removeHandle(); showHandle() }
            ACTION_STOP -> stopSelf()
        }
        return START_STICKY
    }

    override fun onDestroy() { removeHandle(); removePanel(); super.onDestroy() }

    private fun createNotificationChannel() {
        val channel = NotificationChannel(CHANNEL_ID, "Edge Panel Service", NotificationManager.IMPORTANCE_LOW)
    ...
    private fun buildNotification(): Notification {
        ...
        val stopIntent = PendingIntent.getService(this, 1,
            Intent(this, EdgePanelService::class.java).apply { action = ACTION_STOP }, PendingIntent.FLAG_IMMUTABLE)
    ...
    private fun showHandle() {
        ...
        ).apply { this.gravity = gravity; y = handleConfig.offsetY }

        var initialY = 0; var initialTouchY = 0f; var isDragging = false; var touchStartTime = 0L

        handleView!!.setOnTouchListener { _, event ->
            when (event.action) {
                MotionEvent.ACTION_DOWN -> { initialY = params.y; initialTouchY = event.rawY; isDragging = false; touchStartTime = System.currentTimeMillis(); true }
                MotionEvent.ACTION_MOVE -> {
                    val dy = (event.rawY - initialTouchY).toInt()
                    if (!handleConfig.isLocked && Math.abs(dy) > 10) {
                        isDragging = true; params.y = initialY + dy
                        try { windowManager.updateViewLayout(handleView, params) } catch (_: Exception) {}
                    }; true
                }
                MotionEvent.ACTION_UP -> {
                    val elapsed = System.currentTimeMillis() - touchStartTime
                    if (!isDragging && elapsed < 300) { if (currentState == PanelState.HIDDEN) showPanel() else hidePanel() }
                    else if (isDragging) { handleConfig.offsetY = params.y; Prefs.saveHandleConfig(this, handleConfig) }
                    true
                }
                else -> false
            }
    ...
    private fun removeHandle() {
        handleView?.let { try { windowManager.removeView(it) } catch (_: Exception) {}; handleView = null }
    }
    ...
    private fun setupPanelControls(params: WindowManager.LayoutParams) {
        ...
        panel.findViewById<View>(R.id.btn_fullscreen)?.setOnClickListener {
            if (currentState == PanelState.FULLSCREEN) {
                ...
                params.x = (screenWidth - params.width) / 2; params.y = (screenHeight - params.height) / 2
                params.flags = WindowManager.LayoutParams.FLAG_LAYOUT_IN_SCREEN
                currentState = PanelState.NORMAL
                panel.findViewById<TextView>(R.id.btn_fullscreen)?.text = "⛶"
            } else {
                params.width = screenWidth; params.height = screenHeight
                params.x = 0; params.y = 0
                params.flags = WindowManager.LayoutParams.FLAG_LAYOUT_IN_SCREEN or WindowManager.LayoutParams.FLAG_FULLSCREEN
        ...
        btnMinMax?.setOnClickListener {
            when (currentState) {
                PanelState.NORMAL, PanelState.MAXIMIZED -> {
                    params.width = dpToPx(PANEL_WIDTH_NORMAL.toFloat()).toInt(); params.height = dpToPx(80f).toInt()
                    currentState = PanelState.MINIMIZED; btnMinMax.text = "⬆"
                    panel.findViewById<View>(R.id.panel_content)?.visibility = View.GONE
                }
                PanelState.MINIMIZED -> {
                    params.width = dpToPx(PANEL_WIDTH_MAX.toFloat()).toInt(); params.height = dpToPx(PANEL_HEIGHT_MAX.toFloat()).toInt()
                    params.x = (screenWidth - params.width) / 2; params.y = (screenHeight - params.height) / 2
                    currentState = PanelState.MAXIMIZED; btnMinMax.text = "⬇"
    ...
    private fun setupPanelTabs() {
        ...
        panels.forEach { config ->
            val tab = TextView(this).apply {
                text = config.title; textSize = 12f; setTextColor(Color.WHITE)
                setPadding(dpToPx(12f).toInt(), dpToPx(8f).toInt(), dpToPx(12f).toInt(), dpToPx(8f).toInt())
                setOnClickListener { loadPanel(config, contentContainer) }; tag = config.id
            }
    ...
    private fun loadAppsPanel(container: FrameLayout) {
        ...
        selectedPkgs.forEach { pkg ->
            try {
                ...
                val lp = GridLayout.LayoutParams().apply {
                    width = dpToPx(72f).toInt(); height = dpToPx(88f).toInt()
        ...
        view.findViewById<TextView>(R.id.btn_edit_apps)?.setOnClickListener {
    ...
    private fun loadTasksPanel(container: FrameLayout) {
        ...
        fun renderTasks() {
            ...
            tasks.forEach { task ->
                ...
                cb.text = task.title; cb.isChecked = task.isDone; cb.setTextColor(Color.WHITE)
                cb.setOnCheckedChangeListener { _, checked -> task.isDone = checked; Prefs.saveTasks(this, tasks) }
                row.findViewById<TextView>(R.id.btn_delete_task).setOnClickListener {
                    tasks.remove(task); Prefs.saveTasks(this, tasks); renderTasks()
                }
        ...
        addBtn.setOnClickListener {
            ...
            if (text.isNotEmpty()) { tasks.add(TaskItem(title = text)); Prefs.saveTasks(this, tasks); input.setText(""); renderTasks() }
        }
        container.addView(view)
    }
    ...
    private fun loadPeoplePanel(container: FrameLayout) {
        ...
        Prefs.loadSelectedContacts(this).forEach { id ->
            ...
            if (contact.photoUri != null) {
                try { row.findViewById<ImageView>(R.id.person_photo)?.setImageURI(android.net.Uri.parse(contact.photoUri)) } catch (_: Exception) {}
            }
            row.findViewById<View>(R.id.btn_call)?.setOnClickListener {
                startActivity(Intent(Intent.ACTION_CALL, android.net.Uri.parse("tel:${contact.phone}")).apply { addFlags(Intent.FLAG_ACTIVITY_NEW_TASK) })
            }
            row.findViewById<View>(R.id.btn_sms)?.setOnClickListener {
                startActivity(Intent(Intent.ACTION_SENDTO, android.net.Uri.parse("smsto:${contact.phone}")).apply { addFlags(Intent.FLAG_ACTIVITY_NEW_TASK) })
            }
            list.addView(row)
        }
        view.findViewById<TextView>(R.id.btn_edit_people)?.setOnClickListener {
            startActivity(Intent(this, com.edgepanel.ui.PeoplePanelEditActivity::class.java).apply { addFlags(Intent.FLAG_ACTIVITY_NEW_TASK) })
        }
        container.addView(view)
    }
    ...
    private fun resolveContact(id: String): com.edgepanel.model.PersonItem? {
        return try {
            val cursor = contentResolver.query(android.provider.ContactsContract.Contacts.CONTENT_URI, null, "_id=?", arrayOf(id), null)
            cursor?.use {
                if (it.moveToFirst()) {
                    ...
                    val phoneCursor = contentResolver.query(android.provider.ContactsContract.CommonDataKinds.Phone.CONTENT_URI,
                        null, "${android.provider.ContactsContract.CommonDataKinds.Phone.CONTACT_ID}=?", arrayOf(id), null)
                    val phone = phoneCursor?.use { pc -> if (pc.moveToFirst()) pc.getString(pc.getColumnIndexOrThrow(android.provider.ContactsContract.CommonDataKinds.Phone.NUMBER)) else "" } ?: ""
    ...
    private fun animatePanelIn() {
        panelView?.apply {
            ...
            animate().alpha(1f).scaleX(1f).scaleY(1f).setDuration(220).setInterpolator(DecelerateInterpolator()).start()
        }
    }

    private fun hidePanel() {
        panelView?.animate()?.alpha(0f)?.scaleX(0.85f)?.scaleY(0.85f)?.setDuration(180)?.withEndAction { removePanel() }?.start()
    }

    private fun removePanel() {
        panelView?.let { try { windowManager.removeView(it) } catch (_: Exception) {}; panelView = null }
